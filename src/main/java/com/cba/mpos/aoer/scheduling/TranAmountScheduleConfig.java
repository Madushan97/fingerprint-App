...
@Configuration
@EnableScheduling
public class TranAmountScheduleConfig implements SchedulingConfigurer {

    @Autowired
    AppOperationService appOperationService;

    private final Logger LOGGER = LoggerFactory.getLogger(TranAmountScheduleConfig.class);

    private final TaskConfigRepository taskConfigRepository;

    public TranAmountScheduleConfig(TaskConfigRepository taskConfigRepository) {
    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        taskRegistrar.addTriggerTask(() -> {
            try {
                appOperationService.checkTranAmountAndNotify();
            } catch (Exception e) {
                LOGGER.error("Error occurred while running checkTranAmountAndNotify().", e);
            }
        }, triggerContext -> {
